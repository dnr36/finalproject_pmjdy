...
india_pmjdy %>%
    ggplot() +
    geom_sf(aes(fill = employment), color = "darkorange3", size = 0.1) +
    scale_fill_gradient(
        low = "white", 
        high = "darkgreen",
    ) +
...
# Making a function to make choropleths 
...
india_choropleth <- function(indicator) {
  ...
  india_pmjdy %>%
    ggplot() +
    geom_sf(aes(fill = !!sym(indicator)), color = "darkorange3", size = 0.1) +
    scale_fill_gradient(
        low = "white", 
        high = "darkgreen",
    ) +
    labs(title = paste0(indicator, " across India"),
          fill = indicator) +
    theme_void()
   
}

india_choropleth(indicator = "educationindex")
